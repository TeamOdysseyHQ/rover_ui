...
slot_code = """<script lang="ts">
  ...
  let selectedCamera = $state(slot.cameraId || '');
  let selectedType = $state(slot.streamType || 'mjpeg');
  let selectedResolution = $state(slot.resolution || '720p');
  ...
  async function initStream() {
    ...
    if (!slot.isConfigured || !selectedCamera) return;

    if (slot.streamType === 'mjpeg') {
      if (imgRef) {
        imgRef.src = mjpegUrl();
      }
    } else if (slot.streamType === 'websocket') {
      if (!canvasRef) return;
    ...
    } else if (slot.streamType === 'webrtc') {
      if (!videoRef) return;
  ...
  async function handleConfigure() {
    slot.cameraId = selectedCamera;
    slot.streamType = selectedType;
    slot.resolution = selectedResolution;
...
<div class="relative flex h-full w-full flex-col items-center justify-center border border-slate-700 bg-slate-900 overflow-hidden rounded-md">
  {#if !slot.isConfigured}
    <div class="flex flex-col gap-4 p-6 w-full max-w-sm text-white bg-slate-800 rounded-lg shadow-lg">
      ...
      <div class="flex justify-end gap-3 mt-4">
        {#if onRemove}
          <button onclick={() => onRemove(slot.id)} class="text-red-400 hover:text-red-300 text-sm font-medium px-2">
            Remove Slot
          </button>
        {/if}
        <button 
  ...
  {:else}
    <div class="absolute top-2 left-2 z-10 bg-black/70 px-2 py-1 rounded text-xs font-mono text-green-400 shadow backdrop-blur-sm flex flex-col gap-1">
      <div>{slot.cameraId} | {slot.streamType.toUpperCase()} | FPS: {actualFps}</div>
      {#if streamError}
    ...
    <div class="absolute top-2 right-2 z-10 flex gap-2">
       <button 
         ...
         onclick={() => { slot.isConfigured = false; }}>
         Edit
       </button>
    </div>
...
with open("src/lib/components/panels/FullscreenCameraSlot.svelte", "w") as f:
    f.write(slot_code)

with open("src/lib/components/panels/FullscreenCameraView.svelte", "w") as f:
    f.write(view_code)

print("Done writing Svelte components.")
